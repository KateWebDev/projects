...
export default function App() {
  ...
  function activeTabs(numTab) {
    setActiveTab(numTab);
  }

  return (
    <>
      {!isOpen && <button onClick={show}>Start</button>}
      ...
      {isOpen && (
        <div className="app">
          ...
          <div className="tab-buttons">
            <button onClick={() => activeTabs(0)} className={`tab-button ${activeTab === 0 ? "active" : ""}`}>
              Tab 1
            </button>
            <button onClick={() => activeTabs(1)} className={`tab-button ${activeTab === 1 ? "active" : ""}`}>
              Tab 2
            </button>
            <button onClick={() => activeTabs(2)} className={`tab-button ${activeTab === 2 ? "active" : ""}`}>
              Tab 3
            </button>
          </div>
          ...
          <div className="navigation-buttons">
            <button
              onClick={() => (activeTab > 0 ? setActiveTab((tab) => tab - 1) : activeTab)}
              disabled={activeTab === 0}
            >
              &lt; Previous
            </button>
            <button
              onClick={() => (activeTab < tabData.length - 1 ? setActiveTab((tab) => tab + 1) : activeTab)}
